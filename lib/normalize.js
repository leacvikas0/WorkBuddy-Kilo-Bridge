...
function buildUpstreamBody(inBody) {
  ...
  if (maxTokens !== undefined && maxTokens !== null) {
    ...
    let resolved = Number.isFinite(num) ? Math.round(num) : maxTokens;
    // If client transmits legacy 32k ceiling (e.g. 32000 or 32768), expand it so reasoning never suffocates
    if (typeof resolved === 'number' && (resolved === 32000 || resolved === 32768)) {
      resolved = 131072;
    }
    // Upstream Tencent supports a 128k completion token ceiling (131072 tokens)
    out.max_tokens = typeof resolved === 'number' && resolved > 131072 ? 131072 : resolved;
  } else {
    out.max_tokens = 131072;
  }
  ...
  if (inBody.thinking !== undefined && inBody.thinking !== null) {
    if (typeof inBody.thinking === 'object') {
      out.thinking = { ...inBody.thinking };
      if (out.thinking.budgetTokens === 32000 || out.thinking.budgetTokens === 32768) {
        out.thinking.budgetTokens = 120000;
      }
      if (out.thinking.budget_tokens === 32000 || out.thinking.budget_tokens === 32768) {
        out.thinking.budget_tokens = 120000;
      }
    } else {
      out.thinking = inBody.thinking;
    }
  ...
  if (inBody.budget_tokens !== undefined && inBody.budget_tokens !== null) {
    let b = Number(inBody.budget_tokens);
    if (Number.isFinite(b) && b > 0) {
      if (b === 32000 || b === 32768) b = 120000;
      out.budget_tokens = Math.round(b);
    }
  }
  return out;
}
